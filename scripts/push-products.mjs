...
// expects: one "products:catalog" record (metadata only) plus one
// "products:quarter:<period>" record per quarter (metrics keyed by product name).
...
const items = { "products:catalog": catalog };
for (const quarter of staticProducts.quarters) {
  ...
  items[`products:quarter:${quarter}`] = { generatedAt: staticProducts.generatedAt, metrics };
}

await upsertItems(items);

console.log(`Pushed products:catalog + ${staticProducts.quarters.length} quarter record(s) to Global Config.`);
